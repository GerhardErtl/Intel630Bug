...
void ContextManager::DestroyAllContexts(void)
{
    for (uint32_t i = 0; i < 4; ++i)
    {
        ...
        while(!m_AvailableContexts[i].empty())
        {
            m_AvailableContexts[i].pop();
        }
    }
}

CommandContext* ContextManager::AllocateContext(D3D12_COMMAND_LIST_TYPE Type, GraphicsCore& core, const std::wstring ID)
{
    ...
    {
        ret = new CommandContext(Type, core);
...
CommandContext& CommandContext::Begin( const std::wstring ID, GraphicsCore& core)
{
    CommandContext* NewContext = core.m_pContextManager->AllocateContext(D3D12_COMMAND_LIST_TYPE_DIRECT, core, ID);
    NewContext->SetID(ID);
    //if (ID.length() > 0)
        //EngineProfiling::BeginBlock(ID, NewContext);
        
    NewContext->PIXBeginEvent();
    return *NewContext;
}

ComputeContext& ComputeContext::Begin(const std::wstring& ID, GraphicsCore& core, bool Async)
{
    ComputeContext& NewContext = core.m_pContextManager->AllocateContext(
        Async ? D3D12_COMMAND_LIST_TYPE_COMPUTE : D3D12_COMMAND_LIST_TYPE_DIRECT, core, ID)->GetComputeContext();
    NewContext.SetID(ID);
    //if (ID.length() > 0)
        //EngineProfiling::BeginBlock(ID, &NewContext);
    return NewContext;
...
uint64_t CommandContext::Flush(bool WaitForCompletion)
{
    ...
    uint64_t FenceValue = this->m_Core.m_pCommandManager->GetQueue(m_Type).ExecuteCommandList(m_CommandList);

    if (WaitForCompletion)
        this->m_Core.m_pCommandManager->WaitForFence(FenceValue);
...
//    if (m_ID.length() > 0)
    ...
    CommandQueue& Queue = this->m_Core.m_pCommandManager->GetQueue(m_Type);
    ...
    this->m_Core.m_pContextManager->FreeContext(this);

    return FenceValue;
}

CommandContext::CommandContext(D3D12_COMMAND_LIST_TYPE Type, GraphicsCore& core) :
    m_Core(core),
    m_Type(Type),
    m_DynamicViewDescriptorHeap(*this, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, *core.m_pDynamicDescriptorHeapStatics),
    m_DynamicSamplerDescriptorHeap(*this, D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, *core.m_pDynamicDescriptorHeapStatics),
    m_CpuLinearAllocator(kCpuWritable, *core.m_pLinearAllocatorStatics),
    m_GpuLinearAllocator(kGpuExclusive, *core.m_pLinearAllocatorStatics)
{
...
{
    this->m_Core.m_pCommandManager->CreateNewCommandList(m_Type, &m_CommandList, &m_CurrentAllocator, ID);
}

void CommandContext::Reset(const std::wstring ID)
{
    // We only call Reset() on previously freed contexts.  The command list persists, but we must
    // request a new allocator.
    ASSERT(m_CommandList != nullptr && m_CurrentAllocator == nullptr);
    auto& queue = this->m_Core.m_pCommandManager->GetQueue(m_Type);
    m_CurrentAllocator = queue.RequestAllocator();
    m_CommandList->Reset(m_CurrentAllocator, nullptr);
    m_CommandList->SetName(ID.c_str());
    queue.m_CommandQueue->SetName(ID.c_str());
...
void CommandContext::InitializeTexture(GraphicsCore& core, GpuResource& Dest, UINT NumSubresources, D3D12_SUBRESOURCE_DATA SubData[] )
{
    UINT64 uploadBufferSize = GetRequiredIntermediateSize(Dest.GetResource(), 0, NumSubresources);

    CommandContext& InitContext = CommandContext::Begin(L"InitializeTexture", core);
...
void CommandContext::InitializeTextureArraySlice(GraphicsCore& core, GpuResource& Dest, UINT SliceIndex, GpuResource& Src)
{
    CommandContext& Context = CommandContext::Begin(L"InitializeTextureArraySlice", core);
...
void CommandContext::ReadbackTexture2D(GraphicsCore& core, GpuResource& ReadbackBuffer, PixelBuffer& SrcBuffer)
{
    ...
    core.m_pDevice->GetCopyableFootprints(&srcDesc, 0, 1, 0, &PlacedFootprint, nullptr, nullptr, nullptr);

    // This very short command list only issues one API call and will be synchronized so we can immediately read
    // the buffer contents.
    CommandContext& Context = CommandContext::Begin(L"ReadbackTexture2D", core);

    Context.TransitionResource(SrcBuffer, D3D12_RESOURCE_STATE_COPY_SOURCE, true);

    auto dst = CD3DX12_TEXTURE_COPY_LOCATION(ReadbackBuffer.GetResource(), PlacedFootprint);
    auto src = CD3DX12_TEXTURE_COPY_LOCATION(SrcBuffer.GetResource(), 0);
    Context.m_CommandList->CopyTextureRegion(
        &dst, 0, 0, 0,
        &src, nullptr);

    Context.TransitionResource(SrcBuffer, D3D12_RESOURCE_STATE_PRESENT, true);

    Context.Finish(true);
}

void CommandContext::ReadbackTextureLevel2D(GraphicsCore& core, GpuResource& ReadbackBuffer, int mipLevel, PixelBuffer& SrcBuffer)
{
    // The footprint may depend on the device of the resource, but we assume there is only one device.
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT PlacedFootprint;
    auto srcDesc = SrcBuffer.GetResource()->GetDesc();
    ...
    core.m_pDevice->GetCopyableFootprints(&srcDesc, mipLevel, 1, 0, &PlacedFootprint, &numRows, &rowSizeInBytes, &totalBytes);

    // This very short command list only issues one API call and will be synchronized so we can immediately read
    // the buffer contents.
    CommandContext& Context = CommandContext::Begin(L"ReadbackTextureLevel2D", core);

    Context.TransitionResource(SrcBuffer, D3D12_RESOURCE_STATE_COPY_SOURCE, true);

    auto dst = CD3DX12_TEXTURE_COPY_LOCATION(ReadbackBuffer.GetResource(), PlacedFootprint);
    auto src = CD3DX12_TEXTURE_COPY_LOCATION(SrcBuffer.GetResource(), mipLevel);
    Context.m_CommandList->CopyTextureRegion(
        &dst, 0, 0, 0,
        &src, nullptr);
...
void CommandContext::InitializeBuffer(const std::wstring& name, GraphicsCore& core, GpuResource& Dest, const void* BufferData, size_t NumBytes, size_t Offset)
{
    CommandContext& InitContext = CommandContext::Begin(L"InitializeBuffer: " + name, core);
...
void CommandContext::PIXBeginEvent()
{
}
