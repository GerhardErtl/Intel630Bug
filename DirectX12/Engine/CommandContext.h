...
struct DynamicDescriptorHeapStatics;
class GraphicsCore;
...
public:
    ...
    CommandContext* AllocateContext(D3D12_COMMAND_LIST_TYPE Type, GraphicsCore& core, const std::wstring ID);
...
public:
    ...
    static CommandContext& Begin(const std::wstring ID, GraphicsCore& core);
    ...
    static void InitializeTexture(GraphicsCore& core, GpuResource& Dest, UINT NumSubresources, D3D12_SUBRESOURCE_DATA SubData[] );
    static void InitializeBuffer(const std::wstring& name, GraphicsCore& core, GpuResource& Dest, const void* Data, size_t NumBytes, size_t Offset = 0);
    static void InitializeTextureArraySlice(GraphicsCore& core, GpuResource& Dest, UINT SliceIndex, GpuResource& Src);
    static void ReadbackTexture2D(GraphicsCore& core, GpuResource& ReadbackBuffer, PixelBuffer& SrcBuffer);
    static void ReadbackTextureLevel2D(GraphicsCore& core, GpuResource& ReadbackBuffer, int mipLevel, PixelBuffer& SrcBuffer);
...
public:
    ...
    static GraphicsContext& Begin(const std::wstring& ID, GraphicsCore& core)
    {
        return CommandContext::Begin(ID, core).GetGraphicsContext();
    }
...
{
public:
    static ComputeContext& Begin(const std::wstring& ID, GraphicsCore &core, bool Async = false);
...
inline void CommandContext::SetPipelineState( const PSO& PSO )
{
    ...
    if (PipelineState == this->m_CurPipelineState)
    {
        return;
    }

    this->m_CommandList->SetPipelineState(PipelineState);
    this->m_CurPipelineState = PipelineState;
}
...
{
    ExecuteIndirect(this->m_Core.DrawIndirectCommandSignature, ArgumentBuffer, ArgumentBufferOffset);
}
...
{
    ExecuteIndirect(this->m_Core.DispatchIndirectCommandSignature, ArgumentBuffer, ArgumentBufferOffset);
}
