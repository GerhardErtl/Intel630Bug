...
void RootSignature::Finalize(ID3D12Device4*& device, const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    if (m_Finalized)
        return;
    ...
    bool firstCompile = false;
    {
        ...
        auto iter = this->m_core.m_RootSignatureHashMap.find(HashCode);

        // Reserve space so the next inquiry will find that someone got here first.
        if (iter == this->m_core.m_RootSignatureHashMap.end())
        {
            RSRef = this->m_core.m_RootSignatureHashMap[HashCode].GetAddressOf();
            firstCompile = true;
        }
        else
    ...
    if (firstCompile)
    {
        ...
        this->m_core.m_RootSignatureHashMap[HashCode].Attach(m_Signature);
