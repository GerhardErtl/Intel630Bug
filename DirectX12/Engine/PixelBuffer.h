...
{
public:
    PixelBuffer(GraphicsCore& core, DXGI_FORMAT format)
    : GpuResource(core), m_Width(0), m_Height(0), m_ArraySize(0), m_Format(format), m_BankRotation(0) {}

    uint32_t GetWidth(void) const { return this->m_Width; }
    uint32_t GetHeight(void) const { return this->m_Height; }
    uint32_t GetDepth(void) const { return this->m_ArraySize; }
    const DXGI_FORMAT& GetFormat(void) const { return this->m_Format; }

    // Has no effect on Windows
    void SetBankRotation( uint32_t RotationAmount ) { this->m_BankRotation = RotationAmount; }
...
protected:
    ...
    void CreateSharedTextureResource(const std::wstring& Name,
                                     const D3D12_RESOURCE_DESC& ResourceDesc, D3D12_CLEAR_VALUE ClearValue,
                                     D3D12_GPU_VIRTUAL_ADDRESS = D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN);
