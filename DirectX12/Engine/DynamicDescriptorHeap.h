...
{
public:
    DynamicDescriptorHeapStatics(GraphicsCore& core) : m_core(core) {};

    std::mutex m_Mutex;
    std::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> m_DescriptorHeapPool[2];
    std::queue<std::pair<uint64_t, ID3D12DescriptorHeap*>> m_RetiredDescriptorHeaps[2];
    std::queue<ID3D12DescriptorHeap*> m_AvailableDescriptorHeaps[2];
    GraphicsCore& m_core;
    ...
    void Destroy()
    {
        m_DescriptorHeapPool[0].clear();
        m_DescriptorHeapPool[1].clear();
    }
};
...
{
public:
    DynamicDescriptorHeap(CommandContext& OwningContext, D3D12_DESCRIPTOR_HEAP_TYPE HeapType, DynamicDescriptorHeapStatics &dynamicDescriptorHeapStatics);
    ...
    inline void CommitGraphicsRootDescriptorTables( ID3D12GraphicsCommandList* CmdList )
    {
        if (m_GraphicsHandleCache.m_StaleRootParamsBitMap != 0)
            CopyAndBindStagedTables(m_dynamicDescriptorHeapStatics.m_core, m_GraphicsHandleCache, CmdList, &ID3D12GraphicsCommandList::SetGraphicsRootDescriptorTable);
    }
    ...
    inline void CommitComputeRootDescriptorTables( ID3D12GraphicsCommandList* CmdList )
    {
        if (m_ComputeHandleCache.m_StaleRootParamsBitMap != 0)
            CopyAndBindStagedTables(m_dynamicDescriptorHeapStatics.m_core, m_ComputeHandleCache, CmdList, &ID3D12GraphicsCommandList::SetComputeRootDescriptorTable);
    }

private:
    DynamicDescriptorHeapStatics& m_dynamicDescriptorHeapStatics;
    ...
    struct DescriptorHandleCache
    {
        ...
        void CopyAndBindStaleTables(
            ID3D12Device4* device, D3D12_DESCRIPTOR_HEAP_TYPE Type, uint32_t DescriptorSize, DescriptorHandle DestHandleStart, ID3D12GraphicsCommandList* CmdList,
    ...
    void CopyAndBindStagedTables(GraphicsCore& core, DescriptorHandleCache& HandleCache, ID3D12GraphicsCommandList* CmdList,
        void (STDMETHODCALLTYPE ID3D12GraphicsCommandList::*SetFunc)(UINT, D3D12_GPU_DESCRIPTOR_HANDLE) );
