...
{
public:
    LinearAllocationPage(GraphicsCore& core, ID3D12Resource* pResource, D3D12_RESOURCE_STATES Usage) : GpuResource(core)
    {
...
public:
    ...
    LinearAllocatorPageManager(GraphicsCore &core, LinearAllocatorType);
...
{
public:
    LinearAllocatorStatics(GraphicsCore& core) : m_PageManager{ {core, LinearAllocatorType::kGpuExclusive},
                                                                    {core, LinearAllocatorType::kCpuWritable } }
    {}
public:
    LinearAllocatorPageManager m_PageManager[2];

    void DestroyAll(void)
    {
        m_PageManager[0].Destroy();
        m_PageManager[1].Destroy();
    }
};
...
public:
    ...
    LinearAllocator(LinearAllocatorType Type, LinearAllocatorStatics &linearAllocatorStatics) : 
        m_AllocationType(Type),
        m_PageSize(0),
        m_CurOffset(~(size_t)0),
        m_CurPage(nullptr),
        m_LinearAllocatorStatics(linearAllocatorStatics)
    {
...
private:
    ...
    LinearAllocatorStatics &m_LinearAllocatorStatics;
