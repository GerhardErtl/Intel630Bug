...
void PixelBuffer::AssociateWithResource(const std::wstring& Name, ID3D12Resource* Resource, D3D12_RESOURCE_STATES CurrentState)
{
    ...
    this->m_pResource.Attach(Resource);
    this->m_UsageState = CurrentState;

    this->m_Width = (uint32_t)ResourceDesc.Width;        // We don't care about large virtual textures yet
    this->m_Height = ResourceDesc.Height;
    this->m_ArraySize = ResourceDesc.DepthOrArraySize;
    this->m_Format = ResourceDesc.Format;
    ...
    uint32_t NumMips, DXGI_FORMAT Format, UINT Flags)
{
    ...
    this->m_ArraySize = DepthOrArraySize;
...
void PixelBuffer::CreateTextureResource(const std::wstring& Name,
    const D3D12_RESOURCE_DESC& ResourceDesc, D3D12_CLEAR_VALUE ClearValue, D3D12_GPU_VIRTUAL_ADDRESS /*VidMemPtr*/ )
{
    this->Destroy();

    CD3DX12_HEAP_PROPERTIES HeapProps(D3D12_HEAP_TYPE_DEFAULT);
    ASSERT_SUCCEEDED(this->m_core.m_pDevice->CreateCommittedResource( &HeapProps, D3D12_HEAP_FLAG_NONE,
        &ResourceDesc, D3D12_RESOURCE_STATE_COMMON, &ClearValue, MY_IID_PPV_ARGS(&this->m_pResource) ));

    this->m_UsageState = D3D12_RESOURCE_STATE_COMMON;
...
void PixelBuffer::CreateSharedTextureResource(const std::wstring& Name, const D3D12_RESOURCE_DESC& ResourceDesc, D3D12_CLEAR_VALUE ClearValue, D3D12_GPU_VIRTUAL_ADDRESS /*VidMemPtr*/)
{
    this->Destroy();

    CD3DX12_HEAP_PROPERTIES HeapProps(D3D12_HEAP_TYPE_DEFAULT);
    ASSERT_SUCCEEDED(this->m_core.m_pDevice->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_SHARED,
        &ResourceDesc, D3D12_RESOURCE_STATE_COMMON, &ClearValue, MY_IID_PPV_ARGS(&this->m_pResource)));
...
void PixelBuffer::ExportToFile( const std::wstring& FilePath )
{
    ...
    ReadbackBuffer TempBuffer(this->m_core);

    const UINT bytesPerPixelOfInputBitmap = static_cast<const UINT>(PixelBuffer::BytesPerPixel(m_Format));

    auto rowPitchOfSource = (m_Width * bytesPerPixelOfInputBitmap + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1) / D3D12_TEXTURE_DATA_PITCH_ALIGNMENT;
    rowPitchOfSource = rowPitchOfSource * D3D12_TEXTURE_DATA_PITCH_ALIGNMENT;

    TempBuffer.Create(L"Temporary Readback Buffer", rowPitchOfSource / bytesPerPixelOfInputBitmap * m_Height, bytesPerPixelOfInputBitmap);


    CommandContext::ReadbackTexture2D(this->m_core, TempBuffer, *this);
