...
CommandListManager::CommandListManager(GraphicsCore& core) :
    m_Core(core),
...
void CommandQueue::Create(ID3D12Device* pDevice, D3D12_COMMAND_QUEUE_FLAGS commandQueueFlag)
{
...
{
    m_GraphicsQueue.Create(this->m_Core.m_pDevice, D3D12_COMMAND_QUEUE_FLAGS::D3D12_COMMAND_QUEUE_FLAG_DISABLE_GPU_TIMEOUT);
    m_ComputeQueue.Create(this->m_Core.m_pDevice, D3D12_COMMAND_QUEUE_FLAGS::D3D12_COMMAND_QUEUE_FLAG_DISABLE_GPU_TIMEOUT);
    m_CopyQueue.Create(this->m_Core.m_pDevice, D3D12_COMMAND_QUEUE_FLAGS::D3D12_COMMAND_QUEUE_FLAG_DISABLE_GPU_TIMEOUT);
}

void CommandListManager::CreateNewCommandList( D3D12_COMMAND_LIST_TYPE Type, ID3D12GraphicsCommandList** List, ID3D12CommandAllocator** Allocator, const std::wstring ID)
{
    ...
    ASSERT_SUCCEEDED( this->m_Core.m_pDevice->CreateCommandList(1, Type, *Allocator, nullptr, MY_IID_PPV_ARGS(List)) );
...
{
    CommandQueue& Producer = this->m_Core.m_pCommandManager->GetQueue((D3D12_COMMAND_LIST_TYPE)(FenceValue >> 56));
