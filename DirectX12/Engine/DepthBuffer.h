...
{
public:
    DepthBuffer(GraphicsCore& core, DXGI_FORMAT format,
        float ClearDepth = 0.0f, uint8_t ClearStencil = 0 )
        : PixelBuffer(core, format), m_ClearDepth(ClearDepth), m_ClearStencil(ClearStencil)
    {
    ...
    void Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t NumSamples = 1,
                D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr = D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN);
